...
This prototype implements the two-layer extraction + compliance architecture with 105 tests, `GET /v1/applications` COLA stub catalog, real-label smoke tests, a three-provider fallback chain, and a React + Vite web UI. Mode A application-matching is implemented end-to-end (API + UI COLA stub toggle; COLA on-file integration deferred). Server-side batch upload ([ADR-007](docs/adr/007-batch-processing-design.md)) is not built; a client-side batch UI tab ([ADR-013](docs/adr/013-batch-processing-poc.md)) is implemented (multi-file drop, pairing preview, sequential checks, CSV export). HEIC conversion remains deferred. See [`IMPLEMENTATION_STATUS.md`](IMPLEMENTATION_STATUS.md) for the full accounting.
