...
| Delirium Tremens can — 3-panel hybrid (Option B) | — | `[ ]` NONCOMPLIANT (predicted, R-GW-02) | `front=beer/delirium-tremens-can-front.jpg`, `back=beer/delirium-tremens-can-gws-side.jpg` (GWS + side stitched); genuine "OR TO OPERATE MACHINERY" violation (physical inspection; cf. bottle row above); not yet run against live server |
