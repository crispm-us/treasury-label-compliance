...
The test corpus covers both compliant labels and intentionally defective labels (see below). Compliant synthetic examples are generated by `scripts/generate-synthetic-labels.py`. COLA flat artwork and bottle/can photographs live in each beverage-class directory — see *Official COLA artwork* and *Real label pairs* below.
...
The TTB COLA Public Registry exposes metadata (brand name, class/type, status) to anyone; label artwork may require a TTB industry account to view or download. The flat COLA artwork in this repo was sourced from the COLA database — how it was obtained is less important than that it is authentic submitted label art, not a bottle photograph. Run `scripts/download-cola-labels.py --list-only` to search COLA metadata and find products worth photographing or looking up in the BAM.
...
## Official COLA artwork

Flat label artwork as submitted to TTB's COLA (Certificate of Label Approval) system. These are the most authentic test inputs in the corpus — no photo distortion, EXIF rotation, or lighting artifacts. Distinct from bottle/can photographs in *Real label pairs* below.

| File stem | Beverage | Format | Panels | Notes |
|---|---|---|---|---|
| `beer/evergrain-keg-front` | Beer — keg | JPEG | Front only | Keg collar label; no back panel exists for this format |
| `wine/pinotopia-front` + `pinotopia-back` | Wine | PNG | Front + back | Clean two-panel COLA artwork |
| `spirits/gamma-eta-front` + `gamma-eta-back` | Distilled spirits — whisky | PNG | Front + back | Clean two-panel COLA artwork |

COLA artwork labels are in PNG or JPEG as provided by the submitter. See `docs/DEPLOYMENT_CHECKLIST.md §6` for verified smoke-test results.
...
| Delirium Tremens bottle (imported BE, 8.5% ABV) | `beer/delirium-tremens-bottle-front.jpg` | `beer/delirium-tremens-bottle-back.jpg` | Importer: BBL Inc, Frederick MD; GWS body prints "…OR TO OPERATE MACHINERY" (extra "TO") — a genuine R-GW-02 verbatim violation confirmed by physical inspection, not an extraction error |
| Delirium Tremens can (imported BE, 8.5% ABV) | `beer/delirium-tremens-can-front.jpg` | `beer/delirium-tremens-can-gws.jpg` | 3-panel cylinder; `delirium-tremens-can-side.jpg` has ABV + net contents; `delirium-tremens-can-gws-side.jpg` is a pre-stitched GWS + side panel for use as `back` in the 3-panel hybrid submission (Option B — see FAQ Part I §3) |
